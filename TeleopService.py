...
class TeleopService(ServiceOpenTera):
    def __init__(self, config_man: ConfigManager, service_info: dict):
        ...
        ServiceAccessManager.api_participant_token_key = \
            redis_client.redisGet(RedisVars.RedisVar_ParticipantTokenAPIKey)
        ...
        ServiceAccessManager.api_device_token_key = \
            redis_client.redisGet(RedisVars.RedisVar_DeviceTokenAPIKey)
        ServiceAccessManager.api_device_static_token_key = \
            redis_client.redisGet(RedisVars.RedisVar_DeviceStaticTokenAPIKey)
    ...
    def user_manager_event_received(self, pattern, channel, message):
        ...
        try:
            ...
            print('TeleopService - DecodeError ', pattern, channel, message, d)
        except ParseError as e:
            print('TeleopService - Failure in redisMessageReceived', e)

    def handle_user_event(self, event: messages.UserEvent):
        print('TeleopService.handle_user_event', event)
        # Verify each session
    ...
    def handle_participant_event(self, event: messages.ParticipantEvent):
        print('TeleopService.handle_participant_event', event)
        # Verify each session
        for id_session in self.sessions:
            session_info = self.sessions[id_session]
    ...
    def handle_device_event(self, event: messages.DeviceEvent):
        print('TeleopService.handle_device_event', event)
        # Verify each session
        for id_session in self.sessions:
            session_info = self.sessions[id_session]
